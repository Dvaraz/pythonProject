...
def random_point(min_long=-180, max_long=180, min_lat=-90, max_lat=90):
    ...
    if -180 < point[0] - 3 < 180 and -180 < point[0] + 3 < 180:
        min_long = point[0] - 3
        max_long = point[0] + 3
    else:
        random_point()
    if -90 < point[1] - 3 < 90 and -90 < point[1] + 3 < 90:
        min_lat = point[1] - 3
        max_lat = point[1] + 3
    else:
        random_point()
...
def polygon_generator(number):
	for i in range(number):
		a, b, c, d = random_point()
		pts = create_points(10, a, b, c, d)
		# print("Points:", pts)
		hull = graham_scan(pts, False)
		# print("Hull:",hull)
		yield geojson.Polygon([hull])


def create_multipolygon(num_of_polygons):
	multi = geojson.MultiPolygon([x for x in polygon_generator(num_of_polygons)])
	with open(input("file_name: ") + ".geojson", "w") as f:
		f.write(geojson.dumps(multi))

# create_multipolygon(15)